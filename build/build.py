...
# ------------------------------------------------------------------ ЦУС
# Билдеры ЦУС/SIM/1С:ПБиОТ (до «ВКД» включительно) готовят шаблоны для заявок, которые со
# временем переносятся на основной портал с другой архитектурой. Для самого переноса эти
# билдеры и types/* не нужны — нужны только уже собранные файлы из app/data/templates/ и
# fill_template(). Подробности, таблицы токенов и особенности каждого бланка — в PORTING.md
# в корне репозитория; сюда возвращаются, только если исходный бланк придётся пересобрать.
def build_tsus():
